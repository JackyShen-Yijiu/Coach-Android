	...
	public void onDraw(Canvas canvas) {
		...
		if (resultBitmap != null) {
			...
			canvas.drawRect(frame.left - CORNER_WIDTH + 2, frame.top
					- CORNER_WIDTH + 2, frame.left + ScreenRate - CORNER_WIDTH
					+ 2, frame.top + 2, paint);
			canvas.drawRect(frame.left - CORNER_WIDTH + 2, frame.top
					- CORNER_WIDTH + 2, frame.left + 2, frame.top + ScreenRate
			...
			canvas.drawRect(frame.left - CORNER_WIDTH + 2, frame.bottom - 2,
					frame.left + ScreenRate - CORNER_WIDTH + 2, frame.bottom
							+ CORNER_WIDTH - 2, paint);
			...
			/**
			 * 不显示关键点 Collection<ResultPoint> currentPossible =
			 * possibleResultPoints; Collection<ResultPoint> currentLast =
			 * lastPossibleResultPoints; if (currentPossible.isEmpty()) {
			 * lastPossibleResultPoints = null; } else { possibleResultPoints =
			 * new HashSet<ResultPoint>(5); lastPossibleResultPoints =
			 * currentPossible; paint.setAlpha(OPAQUE);
			 * paint.setColor(resultPointColor); for (ResultPoint point :
			 * currentPossible) { canvas.drawCircle(frame.left + point.getX(),
			 * frame.top + point.getY(), 6.0f, paint); } } if (currentLast !=
			 * null) { paint.setAlpha(OPAQUE / 2);
			 * paint.setColor(resultPointColor); for (ResultPoint point :
			 * currentLast) { canvas.drawCircle(frame.left + point.getX(),
