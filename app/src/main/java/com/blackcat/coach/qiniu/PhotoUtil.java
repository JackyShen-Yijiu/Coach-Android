...
public class PhotoUtil {
	 ...
	 * 
	 * @param f 存储图片的file
	 * @param outputX 这里默认是1：1宽高 如果设置为0 会有默认赋值600
	 * @param outputY
	 * @return
	 */
	public static Intent getPictureFromGalleryAndCrop(File f, int outputX, int outputY) {
		Uri uri = Uri.fromFile(f);
	 ...
	 * 返回之后通过BitmapFactory.decodeStream(getContentResolver().openInputStream(uri));获取bitmap
	 * 
	 * @param f
	 * @param outputX  输出图片的宽高 这里默认比例是1：1 所以x y值应该相等 如果设置为0 xy为默认值600
	 * @param outputY
	 * @return
	 */
