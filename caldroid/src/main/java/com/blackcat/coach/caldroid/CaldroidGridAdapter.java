...
public class CaldroidGridAdapter extends BaseAdapter {
    ...
    protected void customizeTextView(int position, CellView cellView) {
        ...
        if (selectedDates != null && selectedDatesMap.containsKey(dateTime)) {
            cellView.setCellViewState(CellView.STATE_SELECTED);
        }

        else if (dateTime.equals(getToday())) {
            cellView.setCellViewState(CellView.STATE_TODAY);
        }

        // Set color of the dates in previous / next month
        else if (dateTime.getMonth() != month) {
            cellView.setCellViewState(CellView.STATE_PREV_NEXT_MONTH);
        }

        // Customize for disabled dates and date outside min/max dates
        else if ((minDateTime != null && dateTime.lt(minDateTime))
                || (maxDateTime != null && dateTime.gt(maxDateTime))
                || (disableDates != null && disableDatesMap
                .containsKey(dateTime))) {

            cellView.setCellViewState(CellView.STATE_DISABLED);
        }

        else {
            cellView.setCellViewState(CellView.STATE_NORMAL);
        }
